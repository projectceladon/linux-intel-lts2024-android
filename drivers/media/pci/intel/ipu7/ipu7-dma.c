...
static void ipu7_dma_sync_single_for_cpu(struct device *dev,
...
static void ipu7_dma_sync_sg_for_cpu(struct device *dev,
...
static void *ipu7_dma_alloc(struct device *dev, size_t size,
...
static void ipu7_dma_free(struct device *dev, size_t size, void *vaddr,
...
static int ipu7_dma_mmap(struct device *dev, struct vm_area_struct *vma,
...
static void ipu7_dma_unmap_sg(struct device *dev,
			      struct scatterlist *sglist,
			      int nents, enum dma_data_direction dir,
			      unsigned long attrs)
{
	...
	if ((attrs & DMA_ATTR_SKIP_CPU_SYNC) == 0)
		ipu7_dma_sync_sg_for_cpu(dev, sglist, nents, DMA_BIDIRECTIONAL);
...
static int ipu7_dma_map_sg(struct device *dev, struct scatterlist *sglist,
			   int nents, enum dma_data_direction dir,
			   unsigned long attrs)
{
	...
	for_each_sg(sglist, sg, count, i) {
		...
		if (ret) {
			ipu7_dma_unmap_sg(dev, sglist, i, dir, attrs);
			return ret;
		}
 ...
 */
static int ipu7_dma_get_sgtable(struct device *dev, struct sg_table *sgt,
				...
				unsigned long attrs)
{
	...
	ret = sg_alloc_table_from_pages(sgt, info->pages, n_pages, 0, size,
					GFP_KERNEL);
	if (ret)
		dev_warn(dev, "IPU get sgt table failed\n");

	return ret;
}

const struct dma_map_ops ipu7_dma_ops = {
	.alloc = ipu7_dma_alloc,
	.free = ipu7_dma_free,
	.mmap = ipu7_dma_mmap,
	.map_sg = ipu7_dma_map_sg,
	.unmap_sg = ipu7_dma_unmap_sg,
	.sync_single_for_cpu = ipu7_dma_sync_single_for_cpu,
	.sync_single_for_device = ipu7_dma_sync_single_for_cpu,
	.sync_sg_for_cpu = ipu7_dma_sync_sg_for_cpu,
	.sync_sg_for_device = ipu7_dma_sync_sg_for_cpu,
	.get_sgtable = ipu7_dma_get_sgtable,
};
