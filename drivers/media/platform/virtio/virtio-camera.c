...

{
	...
	if (vb2_get_num_buffers(vq) + *nbuffers < 2)
		*nbuffers = 2 - vb2_get_num_buffers(vq);
...
static int vcam_buf_init(struct vb2_buffer *vb)
{
	...
	if (strcmp(vb->vb2_queue->dev->driver->name, "virtio-ivshmem") == 0 ||
		...
		pr_err("%s: ivshmem is not supported for now\n",  __func__);
				...
				struct virtio_camera *vcam)
{
	...
	vqs_info = kmalloc_array(vcam->nvqs, sizeof(vqs_info[0]), GFP_KERNEL);
	if (!vqs || !vqs_info) {
		pr_err("%s: virtio_camera failed alloc vq mem.\n", __func__);
		if (!vqs)
			kfree(vqs);
		if (!vqs_info)
			kfree(vqs_info);
	...
	for (i = 0; i < vcam->nvqs; i++) {
		snprintf(vcam->vqs[i].name, VQ_NAME_LEN, "cam_stream.%u", i);
		vqs_info[i].name = vcam->vqs[i].name;
		vqs_info[i].callback = virtio_camera_control_ack;
	}

	ret = virtio_find_vqs(vdev, vcam->nvqs, vqs, vqs_info, NULL);
	if (ret < 0) {
				...
				struct virtio_camera *vcam)
{
	...
	for (i = 0; i < vcam->config.num_virtual_cameras; i++) {
		...
		for (j = 0; j < vcam->config.nr_per_virtual_camera[i]; j++) {
			...
			vnode->vb_queue.min_queued_buffers = 1;
